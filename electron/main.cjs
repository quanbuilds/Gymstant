  ...
  try {
    ...
    while ((step = taskRuntime.pending(task))) {
      ...
      const { stdout } = await runProcessGroup(gymstantCliPath(), ['-z', prompt, '-t', 'computer_use'], {
        // Hermes owns the computer-use session. Its browser/app focus changes
        // are not a human interruption; the old focus monitor saw Safari (or
        // Chrome) and repeatedly SIGSTOP/SIGCONT'd Hermes until the pass
        // timed out. Human interruption is still handled by the explicit Stop
        // control and the execution pill.
        cwd: gymstantWorkdir(), timeout: 300000, progressTimeout: 0, focusAware: false, maxBuffer: 1024 * 1024,
        env: hermesEnv()
      });
